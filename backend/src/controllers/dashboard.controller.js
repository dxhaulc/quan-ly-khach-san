...
const process7DaysChart = (sqlRecords) => {
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    const record = sqlRecords.find(r => 
      new Date(r.PaymentDate).toISOString().split("T")[0] === sqlDateMatch
    );
...
const processMonthChart = (sqlRecords) => {
  const now = new Date();
...
const getDashboardData = async (req, res) => {
  ...
  try {
    ...
    const [summaryRes, chart7DaysRes, chartMonthRes] = await Promise.all([
      request.query(`
        SELECT 
          ...
          ISNULL(SUM(CASE WHEN MONTH(i.PaymentDate) = MONTH(GETDATE()) AND YEAR(i.PaymentDate) = YEAR(GETDATE()) THEN i.TotalAmount + ISNULL(b.DepositAmount, 0) ELSE 0 END), 0) AS MonthRevenue
        FROM Invoices i
      ...
      request.query(`
        ...
        INNER JOIN Bookings b ON i.BookingId = b.Id
        WHERE i.TenantId = @tenantId AND b.BranchId = @branchId
          AND MONTH(i.PaymentDate) = MONTH(GETDATE()) AND YEAR(i.PaymentDate) = YEAR(GETDATE())
    ...
    res.json({
      ...
      chartMonth: processMonthChart(chartMonthRes.recordset),
    });
